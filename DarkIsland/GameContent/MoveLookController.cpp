...
void MoveLookController::InitWindow(_In_ CoreWindow^ window)
{
	...
	window->PointerWheelChanged +=
		ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &MoveLookController::OnWheelChanged);
...
//----------------------------------------------------------------------
bool MoveLookController::IsSwitching()
{
...
bool MoveLookController::IsJumping()
{
	if (m_state == MoveLookControllerState::Active)
	{
		if (m_jumpPressed)
...
bool MoveLookController::IsGrabbing()
{
	if (m_mouseMiddleInUse == true)
	{
		return true;
	}
	else
...
bool MoveLookController::IsRightClick()
{
	if (m_mouseRightInUse == true)
	{
		return true;
	}
	else
...
bool MoveLookController::IsFiring()
{
	if (m_state == MoveLookControllerState::Active)
	{
		if (false)//KeyState(Windows::System::VirtualKey::Shift, false) == true)//m_autoFire)
		{
		...
		else
		{
			...
			if (m_firePressed)
			{
				if (KeyState(Windows::System::VirtualKey::Shift, false) == false)
				{
					m_firePressed = false;
				}
				return true;
...
void MoveLookController::OnWheelChanged(
	_In_ CoreWindow^ /* sender */,
	_In_ PointerEventArgs^ args
)
{
	//args->CurrentPoint->Properties->
	wheelDelta = args->CurrentPoint->Properties->MouseWheelDelta / 120;
...
)
{
	...
	case MoveLookControllerState::Active:
		...
		default:
			...
			bool middleButton = pointProperties->IsMiddleButtonPressed;
			...
			if (!m_mouseInUse)
			{
				...
				m_mouseMiddleInUse = middleButton;
...
)
{
	...
	case MoveLookControllerState::Active:
		...
		else if (pointerID == m_mousePointerID)
		{
			...
			m_mouseMiddleInUse = pointProperties->IsMiddleButtonPressed;
...
void MoveLookController::UpdatePollingDevices()
{
	...
	case MoveLookControllerState::Active:
		...
		//if(reading.Buttons)
		if ((reading.Buttons & GamepadButtons::LeftShoulder) == GamepadButtons::LeftShoulder || KeyState(Windows::System::VirtualKey::Shift, false) == true)
		{
			m_fast = true;
			/*
			if (m_gamepadRunInUse == false)
			{
				m_fast = true;
				//m_gamepadRunInUse = true;
			}
			else
			{
				//m_gamepadRunInUse = false;
			}*/
		}
		else
		{
		...
		{
			if (m_gamepadTorchInUse == false)
			{
				m_torchPressed = true;
				m_gamepadTorchInUse = true;
			}
		}
		else
		{
			m_gamepadTorchInUse = false;
		}

		if ((reading.Buttons & GamepadButtons::A) == GamepadButtons::A)
		{
			if (m_gamepadJumpInUse == false)
			{
				m_jumpPressed = true;
				m_gamepadJumpInUse = true;
			}
		}
		else
		{
			m_gamepadJumpInUse = false;
		}

		if ((reading.Buttons & GamepadButtons::RightShoulder) == GamepadButtons::RightShoulder)
		{
			if (m_gamepadTriggerInUse == false || (reading.Buttons & GamepadButtons::LeftShoulder) == GamepadButtons::LeftShoulder)
			{
				m_firePressed = true;
			}
...
XMFLOAT3 MoveLookController::UpDirection()
{
	XMFLOAT3 lookDirection;

	float r = cosf(m_pitch + (0.5f*M_PI));              // In the plane.
	lookDirection.y = sinf(m_pitch + (0.5f*M_PI));      // Vertical.
	lookDirection.z = r * cosf(m_yaw);    // Fwd-back.
	lookDirection.x = r * sinf(m_yaw);    // Left-right.
...
void MoveLookController::ResetKeyState()
{
	int i;
...
bool MoveLookController::KeyState(Windows::System::VirtualKey key, bool _reset)
{
...
void MoveLookController::Update()
{
	...
	if (m_fast == true || m_gamepadRunInUse == true)
	{
		if (bEditMode == true)
		{
			add_speed = 4.6f;
	...
	XMFLOAT3 wCommand;
	wCommand.x = (m_moveCommand.x * cosf(m_yaw) - m_moveCommand.y * sinf(m_yaw))*add_speed;
	wCommand.y = (m_moveCommand.x * sinf(m_yaw) + m_moveCommand.y * cosf(m_yaw))*add_speed;
	wCommand.z = (m_moveCommand.z)*add_speed;
